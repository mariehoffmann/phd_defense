...
\begin{itemize}
    ...
    \item Missing ground truth % => need to remain sensitive, unbiased
    % \item Taxonomic heterogeneity (show right tax tree)
    \item Databases: sparse, erroneous, uncurated % partial sequences, rarely complete genomes
    \item No unique, generally accepted taxonomic tree!  % if consulting multiple databases to maximize ID
...
%Challenge: amount of primer candidates derivable from database
    ...
    %     \includegraphics[scale=.3]{sarscov2_genome}\caption{Transcript locations of {\it de novo} primers.}
    % \end{figure}
    TODO: illustration
